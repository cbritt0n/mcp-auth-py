...
def test_google_provider_valid(monkeypatch):
    ...
    monkeypatch.setattr("mcp_auth.providers.google.id_token.verify_oauth2_token", fake_verify)
    # ensure google_requests.Request exists
    monkeypatch.setattr("mcp_auth.providers.google.google_requests", types.SimpleNamespace(Request=lambda: None))
...
def test_google_provider_invalid(monkeypatch):
    ...
    monkeypatch.setattr("mcp_auth.providers.google.id_token.verify_oauth2_token", fake_verify_raise)
    monkeypatch.setattr("mcp_auth.providers.google.google_requests", types.SimpleNamespace(Request=lambda: None))
...
def test_aws_provider_jwks(monkeypatch):
    ...
    provider = AWSProvider({"cognito_region": "us-west-2", "cognito_user_pool_id": "pool"})
    ...
    monkeypatch.setattr("mcp_auth.providers.aws.AWSProvider._get_jwks_cache", lambda self: FakeCache())

    # mock jose.jwt.decode to return claims
    monkeypatch.setattr("mcp_auth.providers.aws.jwt.decode", lambda token, jwks, algorithms, options, audience=None: {"sub": "aws-user"})
...
def test_aws_provider_cognito_get_user(monkeypatch):
    ...
    provider = AWSProvider({"use_cognito_get_user": True, "cognito_region": "us-east-1"})
...
def test_azure_provider_jwks(monkeypatch):
    ...
    monkeypatch.setattr("mcp_auth.providers.azure.jwt.decode", lambda token, jwks, algorithms, options, audience=None: {"sub": "azure-user"})
