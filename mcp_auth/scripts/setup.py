...
def create_env_file(provider: str, config: dict):
    ...
    jwt_secret = config.get(
        "jwt_secret", "change-this-in-production-" + os.urandom(8).hex()
    )
...
def setup_azure():
    ...
    config = {
        "provider_config": {"tenant": tenant_id, "audience": client_id},
        "redis_jwks": redis_jwks,
        "redis_url": redis_url,
    }
...
def main():
    ...
    providers = {"1": setup_local, "2": setup_google, "3": setup_aws, "4": setup_azure}
